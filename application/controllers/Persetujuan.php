...
class Persetujuan extends CI_Controller
{
  ...
  public function detail_capaian($kegiatan_id = null, $sk_id = null, $nik = null)
  {
    // check id
    if (!isset($kegiatan_id)) redirect('auth/blocked');
    if (!isset($sk_id)) redirect('auth/blocked');
    ...
    if ($validation->run()) {
      ...
      redirect('persetujuan/detail-petugas/' . $kegiatan_id . '/' . $sk_id . '');
    }
    // form
    $this->load->view('template/header');
    $this->load->view('template/sidebar', $data);
  ...
  public function edit($kegiatan_id = null, $sk_id = null, $nik = null, $id = null)
  {
    // check id
    if (!isset($kegiatan_id)) redirect('auth/blocked');
    if (!isset($sk_id)) redirect('auth/blocked');
    ...
    if ($validation->run()) {
      ...
      redirect('persetujuan/detail-capaian/' . $kegiatan_id . '/' . $sk_id . '/' . $nik . '');
    }
    // form
    $this->load->view('template/header');
    $this->load->view('template/sidebar', $data);
